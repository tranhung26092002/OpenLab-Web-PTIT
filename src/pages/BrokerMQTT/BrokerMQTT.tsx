...
import { AirOutlined, AirplanemodeActiveOutlined, HighlightOffOutlined, HighlightOutlined, LightModeOutlined, NotificationsActiveOutlined, OpacityOutlined, ThermostatOutlined } from '@mui/icons-material';
...
const MqttClient = () => {
    ...
    const [topicPub, setTopicPub] = useState('mqtt/remote/gateway');
    const [message, setMessage] = useState('');
    const [connectionStatus, setConnectionStatus] = useState('Disconnected');
    const [data, setData] = useState('');
    const [user, setUser] = useState('hungtran'); // Thêm state cho user
    const [password, setPassword] = useState('hungtran'); // Thêm state cho password
    ...
    const onMessageArrived = (message: { payloadString: string }) => {
        ...
        try {
            ...
            // Kiểm tra xem data có chứa mảng data hay không và xử lý mảng đó
            if (data && Array.isArray(data.data) && data.data.length > 0) {
                const nodeData = data.data[0]; // Giả sử chỉ có 1 node trong mảng 'data'
    
                // Lấy các giá trị từ nodeData
                setTemperature(nodeData.temp); // Nhiệt độ
                setHumidity(nodeData.hum);     // Độ ẩm
                setLedStatus(nodeData.led === 1); // Trạng thái LED
    
            } else {
                console.error('Invalid or empty data array.');
            }
    
            // Các giá trị khác có thể không có trong dữ liệu, nếu có thể cần thêm kiểm tra
            setGas(data.gas || 0);  // Gas có thể không có trong data, nên kiểm tra giá trị mặc định
            setLight(data.light || 0);  // Light cũng vậy
            setBuzzerStatus(data.buzzer === 1); // Kiểm tra buzzer
            setFanStatus(data.fan === 1); // Kiểm tra quạt
            setServoStatus(data.servo === 1); // Kiểm tra servo
        } catch (error) {
    ...
    const ActuatorBlock: React.FC<{
        ...
        <div className={styles.Block}>
            {status ? icon : <HighlightOffOutlined fontSize="large" />}
            <div className={styles.Info}>
                <h4>{title}</h4>
            </div>
            <button onClick={() => {
                    ...
                    <div className={styles.Container}>
                        <div className={styles.MainContainer}>
                            ...
                            <div className={styles.SensorGrid}>
                                ...
                                <ActuatorBlock
                                    title="Quạt"
                                    icon={<AirplanemodeActiveOutlined fontSize="large" />}
                                    status={fanStatus}
                                ...
                                <ActuatorBlock
                                    title="Led"
                                    icon={<HighlightOutlined fontSize="large" />}
                                    status={ledStatus}
                        ...
                        <div className={styles.MainContainer}>
                            ...
                            <div className={styles.SensorGrid}>
                                <SensorBlock title={`Nhiệt độ`} topic={topicSub} value={`${temperature} °C`} icon={ThermostatOutlined} />
                                <SensorBlock title={`Độ ẩm`} topic={topicSub} value={`${humidity} %`} icon={OpacityOutlined} />
                                <SensorBlock title={`Khí gas`} topic={topicSub} value={`${gas} ppm`} icon={AirOutlined} />
                                <SensorBlock title={`Ánh sáng`} topic={topicSub} value={`${light} lux`} icon={LightModeOutlined} />
                            </div>
                        </div>
